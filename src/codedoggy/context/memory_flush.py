...
def run_memory_flush(
    ...
    *,
    ...
    processed = process_flush_response(raw, config)
    if processed.kind is not FlushResultKind.ACCEPTED or not processed.content:
        return processed

    written = _write_flush_to_memory(memory_store, processed.content)
    processed.entries_written = written
    return processed


def _write_flush_to_memory(store: Any, content: str) -> int:
