...
- [x] list_dir: gitignore respect + large-dir extension summary
...
- [x] grep: output_mode content | files_with_matches | count
...
- [x] search_replace: unicode confusable / normalized fallback
- [x] search_replace: edit snippet in success payload (line context after edit)
- [x] read_file: PDF / image / pptx (text/meta; honest capability notes)
- [ ] list_dir: gitignore respect, large-dir extension summary (BFS collapse)

## Shell / host

- [x] Background tasks (`is_background`, task_id, kill/output tools)
...
- [x] `enter_plan_mode` / `exit_plan_mode` (session mode + plan file seed)
- [x] `ask_user_question` (callback or structured host prompt)

## Cross-tool product surface (still tool-class, not turn)

- [x] Tool kind filtering for capability modes (expanded kinds)
